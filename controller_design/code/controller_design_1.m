...
hold on
plot(t3, log10(abs(yout.signals.values)));
plot(t3, log10(max(abs(E)))*t3 + -0.9321 - 51*log10(max(abs(E))));
xlabel('Samples K');
legend('log(y(k))', ['log(' num2str(v) ')']);
axis([0 500 -130 20])
...
for i = 1:length(RE)
    % Estimator design
    QE = eye(size(A))*RE(i)*ratio; % Process noise
    [M,P,Z,EE] = dlqe(A,G,C,QE,RE(i));
...
legend('Disturbance input signal', ...
        ['Qw = ' num2str(ratio*RE(1)) ', Rv = '  num2str(RE(1)) ''], ...
        ['Qw = ' num2str(ratio*RE(2)) ', Rv = '  num2str(RE(2)) ''], ...
        ['Qw = ' num2str(ratio*RE(3)) ', Rv = '  num2str(RE(3)) ''], ...
        ['Qw = ' num2str(ratio*RE(4)) ', Rv = '  num2str(RE(4)) ''], ...
         'Location', 'southeast');
xlabel('Time [s]');
ylabel('Output');
axis([0 3 0 16])
...
for i = 1:length(RE)
    % Estimator design
    QE = eye(size(A))*ratio*RE(i); % Process noise
    [M,P,Z,EE] = dlqe(A,G,C,QE,RE(i));
...
legend(p,['Qw = ' num2str(ratio*RE(1)) ', Rv = '  num2str(RE(1)) ''], ...
        ['Qw = ' num2str(ratio*RE(2)) ', Rv = '  num2str(RE(2)) ''], ...
        ['Qw = ' num2str(ratio*RE(3)) ', Rv = '  num2str(RE(3)) ''], ...
        ['Qw = ' num2str(ratio*RE(4)) ', Rv = '  num2str(RE(4)) ''], ...
         'Location', 'northwest');
...
legend( '\rho_v = 10', '\rho_v = 1', ...
        '\rho_v = 0.1','Location', 'northwest');
